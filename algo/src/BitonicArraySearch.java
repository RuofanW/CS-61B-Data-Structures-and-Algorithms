public class BitonicArraySearch{
    ...
    public BitonicArraySearch(int[] number){
    	numbers = number;
    	len = numbers.length;
    }
    ...
    private int binarySearchForTurningPoint2(){
        int low=0;
        int high = len - 1;
        while(low < high)
        {
    ...
    public boolean search(int target){
    	if(indexofturningpt < 0){
    		this.indexofturningpt = this.binarySearchForTurningPoint();
    	}
    	boolean res1 = this.binarySearch(target, 0, indexofturningpt);
